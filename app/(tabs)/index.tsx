import { View, StyleSheet, Button, FlatList, Text, NativeModules, NativeEventEmitter, Platform, PermissionsAndroid, ActivityIndicator } from "react-native";
...
export default function Index() {
  ...
  const addOrUpdatePeripheral = (id: any, updatedPeripheral: any) => {
    // new Map() enables changing the reference & refreshing UI.
    // TOFIX not efficient.
    setPeripherals(map => new Map(map.set(id, updatedPeripheral)));
  };

  const convertHashToHex = (value: any[]) => {
    return value.map(v => v.toString(16).padStart(2, '0')).join('');
  }

  const handleDiscoverPeripheral = (peripheral: { name: string; RawData: any; advertising: { manufacturerData: { bytes: any; }; }; id: any; }) => {
